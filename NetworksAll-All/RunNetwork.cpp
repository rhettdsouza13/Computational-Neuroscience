...
double weightUpdate(double t1, double t2, float currentWeight){
    ...
    if (diff>0){
        return (exp(-diff)/2)/(currentWeight);
    }
    else if (diff<0){
        return -(exp(diff)/2)/(currentWeight);
    }
    ...
    if (!(j%5)){
      for(int pre=0; pre<numOfNodes; pre++){
        for(int post=0; post<numOfNodes; post++){
          ...
          for(int postcount=poststart; postcount<SpikeTimes[pre][post][1].size(); postcount++){
          ...
          for(int precount=prestart; precount<SpikeTimes[pre][post][0].size(); precount++){
